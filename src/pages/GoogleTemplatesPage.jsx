...
export default function GoogleTemplatesPage() {
  ...
  const { assets, setAssets, loading, error } = useAssets({ category: "google-templates", subcategory: activeTab || undefined, search: search || undefined });
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Google Doc and Sheet templates for team use.</p>
        </div>
        {isAdmin && (
          <button onClick={() => setAddOpen(true)} className="inline-flex items-center gap-2 rounded-lg bg-brand-700 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-900">
      ...
      <div className="mt-6">
        {loading ? (
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">{Array.from({ length: 8 }).map((_, i) => (<div key={i} className="animate-pulse overflow-hidden rounded-xl border border-gray-200 bg-white"><div className="aspect-[4/3] bg-gray-200" /><div className="space-y-2 p-4"><div className="h-3 w-16 rounded bg-gray-200" /><div className="h-4 w-3/4 rounded bg-gray-200" /></div></div>))}</div>
        ) : error ? (
          <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-center"><p className="text-sm text-red-600">{error}</p></div>
        ) : !assets.length ? (
          <div className="rounded-lg border border-dashed border-gray-300 p-12 text-center"><FolderOpen className="mx-auto h-10 w-10 text-gray-300" /><p className="mt-3 text-sm font-medium text-gray-500">No templates yet</p></div>
        ) : (
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {assets.map((asset) => (<TemplateCard key={asset.id} asset={asset} onDelete={isAdmin ? setDeleteTarget : undefined} />))}
          </div>
        )}
      </div>
      <AddTemplateModal isOpen={addOpen} onClose={() => setAddOpen(false)} onAssetAdded={(asset) => setAssets((prev) => [asset, ...prev])} />
